...
function GlobalFilter({
    ...
    return (
        <label className="flex gap-x-2 items-baseline">
            ...
            <input
                type="text"
                className="border outline-none rounded-sm shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                value={value || ""}
                onChange={e => {
...
export function StatusPillOrder({ value }) {
    const status = value ? value.toLowerCase() : "unknown";
        ...
        <span
            className={
                classNames(
                    ...
                    status.startsWith("pending") ? "bg-gray-100 text-gray-800" : null,
                    status.startsWith("delivery") ? "bg-yellow-100 text-yellow-800" : null,
...
export function ProductImage({ column, row }) {
    return (
        <div className="flex items-center">
...
export function Actions({ value }) {
    const { openDialog, setOpenDialog } = useStateContext();
    const handleOpen = () => {
        setOpenDialog(prevOpenDialog => !prevOpenDialog);
    }
    return (
        <div className="flex items-center gap-4 text-2xl">
            <Link className='hover:text-gray-400' to={`/admin/product/edit/${value}`}><AiOutlineEdit /></Link>

            <button className='hover:text-gray-400' onClick={handleOpen}><BiTrash /></button>
        </div>
    )
}
