                ...
                {products.map((product) => (
                    <div key={product.id} className='max-w-full flex flex-col md:flex-row'>
                        <div className='flex flex-none relative overflow-hidden object-contain  mx-auto justify-center items-center'>
                            <img className='max-w-[300px] md:h-auto p-8' src={product.image} alt="test" />
                        </div>
                        <div className='px-5 flex flex-col flex-1 pt-1 items-start'>
                            <div className='block w-full text-2xl'>{product.title} </div>
                            ...
                            <div className='block h-36'> Lorem ipsum dolor sit amet, consecteto adipisic elit eiusm tempor incidid ut labore et dolore magna aliqua Ut enim ad minim </div>
                            <div>
                                <button onClick={() => addToCart(product.id, products)} className='hover:bg-lavender duration-300 transition-all from-lavender to-lavender bg-black text-white p-3 w-44'> Add to cart</button>
                            </div>
                        </div>
                    </div>
