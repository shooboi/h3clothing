...
public class Utility {
  ...
  public List<CategoryDTO> convertCategoryDTOFromCategories(List<Category> categories) {
    return categories.stream().map(this::convertCategoryDTOFromCategory)
        .collect(Collectors.toList());
  }
  ...
  public UserInfDTO convertUserInfDTOFromUserInf(User_Info userInfo) {
    return new UserInfDTO(userInfo.getFullName(), userInfo.getPhoneNumber(), userInfo.getAddress(),
        userInfo.isDeleted(), userInfo.getDob());
  }
  ...
  public User_Info convertUserInfFromUserInfDTO(UserInfDTO dto) {
    return new User_Info(dto.getFullName(), dto.getPhoneNumber(), dto.getAddress(), dto.isDeleted(),
        dto.getDob());
  }
  ...
  public List<ThumbnailDTO> convertThumbnailDTOFromThumbnails(
      List<Thumbnail_Image> thumbnailImages) {
    return thumbnailImages.stream().map(this::convertThumbnailDTOFromThumbnail)
        .collect(Collectors.toList());
  }
  ...
  public OrderDTO convertOrderDTOFromOrder(Order order) {
    return new OrderDTO(order.getOrderDate(), order.getTotalAmount(), order.getPaymentMethod(),
        order.getDeliveryAddress(), order.getStatus());
  }
  ...
  public Order convertOrderFromOrderDTO(OrderDTO dto) {
    return new Order(dto.getOrderDate(), dto.getTotalAmount(), dto.getPaymentMethod(),
        dto.getDeliveryAddress(), dto.getStatus());
  }
